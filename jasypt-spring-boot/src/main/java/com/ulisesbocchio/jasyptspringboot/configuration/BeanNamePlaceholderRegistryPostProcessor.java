...
/**
 * Bean Definition Registry Post Processor that looks for placeholders in bean names and
 * resolves them, re-defining those beans with the new names.
 */
@Slf4j
class BeanNamePlaceholderRegistryPostProcessor implements BeanDefinitionRegistryPostProcessor, Ordered {
	...
	public void postProcessBeanDefinitionRegistry(BeanDefinitionRegistry registry) throws BeansException {
		...
		Stream.of(bf.getBeanDefinitionNames())
				...
				.filter(name -> name.matches("\\$\\{[\\w.-]+(?>:[\\w.-]+)?\\}")).forEach(placeholder -> {
					...
					log.debug("Registering new name '{}' for Bean definition with placeholder name: {}", actualName,
							placeholder);
				});
	}
