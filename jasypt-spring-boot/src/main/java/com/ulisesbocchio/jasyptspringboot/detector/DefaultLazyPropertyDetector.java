 ...
 * {@link EncryptablePropertyDetector} bean or initializes a default
 * {@link DefaultPropertyDetector}.
 *
...
public class DefaultLazyPropertyDetector implements EncryptablePropertyDetector {
	...
	public DefaultLazyPropertyDetector(ConfigurableEnvironment environment, String customDetectorBeanName,
			boolean isCustom, BeanFactory bf) {
		singleton = new Singleton<>(() -> Optional.of(customDetectorBeanName).filter(bf::containsBean)
				...
				.orElseGet(() -> {
					if (isCustom) {
						throw new IllegalStateException(String.format(
								"Property Detector custom Bean not found with name '%s'", customDetectorBeanName));
					}
					log.info(
