...
public class EncryptorTest {
	...
	private void setup_keyStringEncryptor() {
		...
		String privateKey = Base64.getEncoder().encodeToString(
				FileCopyUtils.copyToByteArray(new ClassPathResource("private_key.der").getInputStream()));
		String publicKey = Base64.getEncoder().encodeToString(
				FileCopyUtils.copyToByteArray(new ClassPathResource("public_key.der").getInputStream()));
	...
	private void setup_keyStringPemEncryptor() {
		SimpleAsymmetricConfig config = new SimpleAsymmetricConfig();
		String privateKey = FileCopyUtils
				.copyToString(new InputStreamReader(new ClassPathResource("private_key.pem").getInputStream()));
		String publicKey = FileCopyUtils
				.copyToString(new InputStreamReader(new ClassPathResource("public_key.pem").getInputStream()));
	...
	private static String getProviderDetails(Provider provider) {
		...
		return provider.getName() + "\n\t"
				+ provider.keySet().stream().map(k -> ((String) k).toUpperCase())
						.filter(k -> k.startsWith(serviceName.toUpperCase()) && !k.contains(" "))
						.map(k -> k.substring(serviceName.length() + 1)).filter(k -> k.startsWith(type))
	...
	public void test_PBEWITHHMACSHA512ANDAES_256_encryption() {
		...
		final String decrypted = new String(_PBEWITHHMACSHA512ANDAES_256.decrypt(ciphertext),
	...
	public void test_GcmPooledEncryptor_encryption_concurrency() {
		...
		try {
			List<String> cipherText = customThreadPool.submit(() -> IntStream.range(0, 100).boxed().parallel()
					.map(v -> gcmPooledKeyEncryptor.encrypt(message)).collect(Collectors.toList())).get();
			List<String> decrypted = customThreadPool.submit(() -> cipherText.stream().parallel()
					.map(gcmPooledKeyEncryptor::decrypt).collect(Collectors.toList())).get();
